...
public:
  ...
  bool operator()(std::string const& p) const
  {
    ...
    // directory:
    return !(cmPath(this->SourceDir).IsPrefix(path) ||
             cmPath(this->BinaryDir).IsPrefix(path));
  }
                                                      ...
                                                      bool verbose)
{
  ...
  for (std::string const& file : files) {
    ...
    if (!depsFiles.empty()) {
      auto dir = cmPath(file).GetParentPath();
      // Clear the implicit dependency makefile.
      auto depFile = cmPath(dir).Append("compiler_depend.make");
      clearer.Clear(depFile.GetString());

      // Remove the internal dependency check file
      auto internalDepFile =
        cmPath(dir).Append("compiler_depend.internal");
      cmSystemTools::RemoveFile(internalDepFile.GetString());

      // Touch timestamp file to force dependencies regeneration
      auto DepTimestamp = cmPath(dir).Append("compiler_depend.ts");
      cmSystemTools::Touch(DepTimestamp.GetString(), true);
